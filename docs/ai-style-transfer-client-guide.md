...
- 重试成功后,**原任务会从 §2.8 的历史列表里消失**,列表里只留下这条新任务。客户端不用自己去重,直接刷新列表就行。原任务仍然可以用 §2.7 按 ID 查到,只是不再出现在列表里。
...
- **同一个原任务只能重试一次**。重试后它已经被新任务取代,再拿它的 ID 调这个接口会报「该任务已经重试过了」且不扣费。如果新任务又失败了,请用**新任务的 ID** 继续重试 —— 也就是永远拿列表里那条最新的失败任务去重试。
...
**被重试取代的任务不会出现在这个列表里**,`page.total` 也不会算它。所以一次「生成失败 → 用户点重新生成」在列表里始终只占一条,客户端不需要按原图或时间去重。重试成功后直接重新拉第一页即可,不要在本地把旧的失败条目和新任务同时留着。
...
| 重试后历史列表里会同时出现失败的原任务和新任务 | 原任务被新任务取代后不再进列表,一次用户操作只显示一条(见 §2.8) |
